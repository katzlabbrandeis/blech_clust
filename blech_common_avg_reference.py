...
def extract_electrode_features(raw_electrodes, electrode_indices, n_samples=10000):
    ...
    for i, electrode_idx in enumerate(tqdm(electrode_indices)):
        # Get electrode data
        electrode_data = get_electrode_by_name(raw_electrodes, electrode_idx)[:]
        ...
        features[i, 2] = np.median(np.abs(electrode_data - np.median(electrode_data)))  # MAD
        features[i, 3] = np.percentile(np.abs(electrode_data), 95)  # 95th percentile
...
if auto_car_inference:
    ...
    for group_num, group_name in enumerate(all_car_group_names):
        ...
        if len(electrode_indices) < 3:
            print(f"Group {group_name} has fewer than 3 electrodes. Skipping clustering.")
            continue
        ...
        for cluster in unique_clusters:
            ...
            print(f"  Cluster {cluster}: {len(cluster_electrodes)} electrodes - {cluster_electrodes}")
