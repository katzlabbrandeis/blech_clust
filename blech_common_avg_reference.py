...
if auto_car_inference:
    ...
    # Get clustering parameters from sorting_params.json
    clustering_params = metadata_handler.params_dict.get('clustering', {})
    max_components = clustering_params.get('max_components', 10)
    iterations = clustering_params.get('iterations', 100)
    threshold = clustering_params.get('threshold', 1e-3)
    
    print(f"Using clustering parameters: max_components={max_components}, iterations={iterations}, threshold={threshold}")
    ...
    for group_num, group_name in enumerate(all_car_group_names):
        ...
        # Perform clustering
        predictions, model = cluster_electrodes(features, n_components=max_components, 
                                               n_iter=iterations, threshold=threshold)
