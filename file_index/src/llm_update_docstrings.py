...
parser = argparse.ArgumentParser(
    description='Update docstrings in all python files in the specified directories')
parser.add_argument('-y', '--yes', action='store_true',
                    help='Automatically update docstrings without prompting')
...
def get_file_list():
    ...
    sorted_files = [
        file for file in sorted_files if '__init__' not in file.name]
...
def main():
    ...
    for file_path in t:
        ...
        if not args.yes:
            continue_bool, break_bool = check_continue(
                file_path.name, docstring)
            if break_bool:
                break
