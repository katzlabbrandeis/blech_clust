    ...
    def _validate_parameters(self):
        ...
        if self.filter_type in ['highpass', 'bandpass']:
            if self.low_freq is None:
                raise ValueError(f"{self.filter_type} filter requires low_freq")
            if self.low_freq <= 0 or self.low_freq >= nyquist:
                raise ValueError(f"low_freq must be between 0 and {nyquist} Hz")
        
        if self.filter_type in ['lowpass', 'bandpass']:
            if self.high_freq is None:
                raise ValueError(f"{self.filter_type} filter requires high_freq")
            if self.high_freq <= 0 or self.high_freq >= nyquist:
                raise ValueError(f"high_freq must be between 0 and {nyquist} Hz")
        
        if self.filter_type == 'bandpass':
            if self.low_freq >= self.high_freq:
                raise ValueError("low_freq must be less than high_freq for bandpass filter")
