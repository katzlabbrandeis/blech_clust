...
class RawDataLoader:
    ...
    def _initialize_metadata(self):
        ...
        with tables.open_file(self.hdf5_path, 'r') as hf5:
            ...
            if not available_groups:
                raise ValueError(f"No raw data groups found in {self.hdf5_path}")
    ...
    def get_channel_list(self, group: str = 'raw') -> List[str]:
        ...
        if group not in self._channel_info:
            raise ValueError(f"Group '{group}' not found. Available: {list(self._channel_info.keys())}")
    ...
    def get_channel_duration(self, channel: str, group: str = 'raw') -> float:
        ...
        if channel not in self._channel_info[group]:
            raise ValueError(f"Channel '{channel}' not found in group '{group}'")
    ...
    def load_channel_data(self, 
        ...
        if channel not in self._channel_info[group]:
            ...
            raise ValueError(f"Channel '{channel}' not found in group '{group}'. Available: {available}")
    ...
    def load_multiple_channels(self,
        ...
        data_array = np.zeros((len(channels), len(first_data)), dtype=first_data.dtype)
    ...
    def __repr__(self):
        ...
        n_channels = sum(len(channels) for channels in self._channel_info.values())
