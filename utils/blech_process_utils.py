...
def compute_rolling_threshold(
    ...
    """Compute rolling spike-detection thresholds.

    Returns two 1-D numpy arrays: *times_sec* (window mid-points) and
    *thresholds* (µV).
    """
...
def plot_rolling_threshold(
    ...
    """Plot rolling thresholds generated by :func:`compute_rolling_threshold`.

    Parameters
    ----------
    times_sec : array-like
        Window mid-points in seconds.
    thresholds : array-like
        Rolling threshold values (µV).
    ax : matplotlib.axes.Axes, optional
        Axes to plot on. If None, a new figure is created.
    global_threshold : float, optional
        If provided, overlay as a red dashed line for comparison.
    """
    if ax is None:
    ...
    if global_threshold is not None:
        ax.axhline(
            ...
            label=f"Global threshold ({global_threshold:.1f} µV)",
        )
