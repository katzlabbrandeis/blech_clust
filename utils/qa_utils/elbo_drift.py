...
def ridge_plot(
    ...
    assert len(y_list) == len(
        colors), f'len(y_list): {len(y_list)}, len(colors): {len(colors)}'
    assert all([len(x_vec) == len(y) for y in y_list]
               ), f'x_vec: {len(x_vec)}, y_list: {[len(y) for y in y_list]}'
...
ax[2].pcolorfast(bins, np.arange(zscored_hists_pca.shape[0]),
                 zscored_hists_pca.T, cmap='viridis')
ax[2].set_title(
    f'PCA of Spike Histograms, {tot_var_explained:.2f} variance explained')
...
best_change = median_elbo_df.sort_values(
    'elbo', ascending=True).changes.values[0]
...
ax[0, 1].plot(median_elbo_df.changes, median_elbo_df.elbo,
              'r', label='Median ELBO')
ax[0, 1].axvline(best_change, color='black', linestyle='--',
                 label=f'Best Change={best_change}')
