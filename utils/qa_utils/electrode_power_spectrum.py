    ...
    with tables.open_file(hf5_path, 'r') as hf5:
        ...
        try:
            ...
            try:
                ...
                warnings.warn(f"Could not find sampling rate, using default {sampling_rate} Hz")
                
        except AttributeError:
            raise ValueError(f"Electrode {electrode_num} not found in {hf5_path}")
...
def plot_electrode_spectra(hf5_path, output_dir=None, max_samples=100000, nperseg=None):
    ...
    with tables.open_file(hf5_path, 'r') as hf5:
        ...
        for node in raw_nodes:
            if 'electrode' in node._v_name:
                electrode_nums.append(int(node._v_name.replace('electrode', '')))
    ...
    for electrode_num in tqdm(electrode_nums, desc="Processing electrodes"):
        try:
            data, sampling_rate = get_electrode_data(hf5_path, electrode_num, max_samples)
            frequencies, psd = calculate_power_spectrum(data, sampling_rate, nperseg)
    ...
    im = ax3.imshow(band_powers.T, aspect='auto', cmap='viridis', interpolation='nearest')
    ...
    peak_frequencies = [freq[np.argmax(psd[1:])] for freq, psd in zip(all_frequencies, all_psds)]
    
    ax4_twin = ax4.twinx()
    
    line1 = ax4.plot(valid_electrodes, total_powers, 'bo-', label='Total Power', markersize=4)
    ax4.set_xlabel('Electrode Number')
    ax4.set_ylabel('Total Power', color='b')
    ax4.tick_params(axis='y', labelcolor='b')
    
    line2 = ax4_twin.plot(valid_electrodes, peak_frequencies, 'ro-', label='Peak Frequency', markersize=4)
    ...
    with open(summary_path, 'w') as f:
        ...
        f.write(f"Frequency range: {common_freq[1]:.2f} - {common_freq[-1]:.2f} Hz\n\n")
        ...
        f.write(f"Min: {np.min(total_powers):.2e} (Electrode {valid_electrodes[np.argmin(total_powers)]})\n")
        f.write(f"Max: {np.max(total_powers):.2e} (Electrode {valid_electrodes[np.argmax(total_powers)]})\n")
