...
dir_name = easygui.diropenbox(
    msg='Where is the hdf5 file from the first day?', title='First day of data')
os.chdir(dir_name)
...
dir_name = easygui.diropenbox(
    msg='Where is the hdf5 file from the second day?', title='Second day of data')
os.chdir(dir_name)
file_list = os.listdir('./')
hdf5_name = ''
for files in file_list:
...
dir_name = easygui.diropenbox(
    msg='Where do you want to save the held units and plots?', title='Output directory')
os.chdir(dir_name)

# Ask the user for the percentile criterion to use to determine held units
percent_criterion = easygui.multenterbox(msg='What percentile of the intra unit J3 distribution do you want to use to pull out held units?', fields=[
                                         'Percentile criterion (1-100) - lower is more conservative'])
...
for unit1 in range(len(hf51.root.unit_descriptor[:])):
    ...
    if hf51.root.unit_descriptor[unit1]['single_unit'] == 1:
        exec(
            "wf_day1 = hf51.root.sorted_units.unit%03d.waveforms[:]" % (unit1))
        pca = PCA(n_components=3)
        pca.fit(wf_day1)
        pca_wf_day1 = pca.transform(wf_day1)
        intra_J3.append(calculate_J3(pca_wf_day1[:int(
            wf_day1.shape[0]*(1.0/3.0)), :], pca_wf_day1[int(wf_day1.shape[0]*(2.0/3.0)):, :]))
...
for unit2 in range(len(hf52.root.unit_descriptor[:])):
    ...
    if hf52.root.unit_descriptor[unit2]['single_unit'] == 1:
        exec(
            "wf_day2 = hf52.root.sorted_units.unit%03d.waveforms[:]" % (unit2))
        pca = PCA(n_components=3)
        pca.fit(wf_day2)
        pca_wf_day2 = pca.transform(wf_day2)
        intra_J3.append(calculate_J3(pca_wf_day2[:int(
            wf_day2.shape[0]*(1.0/3.0)), :], pca_wf_day2[int(wf_day2.shape[0]*(2.0/3.0)):, :]))
...
for unit1 in range(len(hf51.root.unit_descriptor[:])):
    ...
    if hf51.root.unit_descriptor[unit1]['single_unit'] == 1:
        ...
        for unit2 in range(len(hf52.root.unit_descriptor[:])):
            print(unit1, unit2, len(hf51.root.unit_descriptor[:]), len(
                hf52.root.unit_descriptor[:]))
            if hf52.root.unit_descriptor[unit2] == hf51.root.unit_descriptor[unit1]:
                ...
                if inter_J3[-1] <= np.percentile(intra_J3, percent_criterion):
                    ...
                    plt.plot(
                        t - 15, np.mean(wf_day1[:, ::10], axis=0), linewidth=5.0, color='black')
                    plt.plot(t - 15, np.mean(wf_day1[:, ::10], axis=0) - np.std(
                        wf_day1[:, ::10], axis=0), linewidth=2.0, color='black', alpha=0.5)
                    plt.plot(t - 15, np.mean(wf_day1[:, ::10], axis=0) + np.std(
                        wf_day1[:, ::10], axis=0), linewidth=2.0, color='black', alpha=0.5)
                    plt.xlabel('Time (samples (30 per ms))', fontsize=35)
                    plt.ylabel('Voltage (microvolts)', fontsize=35)
                    plt.ylim([np.min(np.concatenate((wf_day1, wf_day2), axis=0)) -
                             20, np.max(np.concatenate((wf_day1, wf_day2), axis=0)) + 20])
                    plt.title('Unit %i, total waveforms = %i, Electrode: %i, J3: %f' % (unit1, wf_day1.shape[0], hf51.root.unit_descriptor[unit1]['electrode_number'], inter_J3[-1]) + '\n' + 'Single Unit: %i, RSU: %i, FS: %i' % (
                        hf51.root.unit_descriptor[unit1]['single_unit'], hf51.root.unit_descriptor[unit1]['regular_spiking'], hf51.root.unit_descriptor[unit1]['fast_spiking']), fontsize=20)
                    ...
                    plt.plot(
                        t - 15, np.mean(wf_day2[:, ::10], axis=0), linewidth=5.0, color='black')
                    plt.plot(t - 15, np.mean(wf_day2[:, ::10], axis=0) - np.std(
                        wf_day2[:, ::10], axis=0), linewidth=2.0, color='black', alpha=0.5)
                    plt.plot(t - 15, np.mean(wf_day2[:, ::10], axis=0) + np.std(
                        wf_day2[:, ::10], axis=0), linewidth=2.0, color='black', alpha=0.5)
                    plt.xlabel('Time (samples (30 per ms))', fontsize=35)
                    plt.ylabel('Voltage (microvolts)', fontsize=35)
                    ...
                    plt.title('Unit %i, total waveforms = %i, Electrode: %i' % (unit2, wf_day2.shape[0], hf52.root.unit_descriptor[unit2]['electrode_number']) + '\n' + 'Single Unit: %i, RSU: %i, FS: %i' % (
                        hf52.root.unit_descriptor[unit2]['single_unit'], hf52.root.unit_descriptor[unit2]['regular_spiking'], hf52.root.unit_descriptor[unit2]['fast_spiking']), fontsize=20)
                    plt.tick_params(axis='both', which='major', labelsize=32)
                    fig.savefig('Unit%i_and_Unit%i.png' %
                                (unit1, unit2), bbox_inches='tight')
                    plt.close('all')
...
plt.axvline(np.percentile(intra_J3, percent_criterion),
            linewidth=5.0, color='black', linestyle='dashed')
