...
def upload_to_s3(local_directory: str, bucket_name: str, s3_directory: str,
    ...
    try:
        ...
        if uploaded_files:
            ...
            index_html_content = generate_index_html(uploaded_files, s3_directory, bucket_name)
...
def generate_index_html(uploaded_files: list, s3_directory: str, bucket_name: str) -> str:
...
"""
    ...
    for ext, files in files_by_ext.items():
        html += f"    <h2>{ext.upper()} Files</h2>\n"
        html += "    <table>\n"
        html += "        <tr><th>File</th><th>Size</th></tr>\n"
        ...
        for file_info in files:
            ...
            html += f"        <tr><td><a href=\"{filename}\">{filename}</a></td><td>{size_str}</td></tr>\n"
...
def generate_github_summary(upload_results: dict, output_file: str = None, bucket_name: str = None) -> str:
    ...
    for ext, files in files_by_ext.items():
        ...
        files = [f for f in files if os.path.basename(f['local_path']) != 'index.html']
        if not files:
            continue
