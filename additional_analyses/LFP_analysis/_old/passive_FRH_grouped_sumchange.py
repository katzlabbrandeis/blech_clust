...
for animal, cond1_units, cond2_units in zip(dirs_animals_1[0],dirs_animals_1[1],dirs_animals_2[1]):
	...
	plt.plot(time, (sliding_mean(np.mean(dirs_2_spike_rates[unit_2_start:unit_2_start+cond2_units-1],axis=0),window=params[3])-sliding_mean(np.mean(dirs_1_spike_rates[unit_1_start:unit_1_start+cond1_units-1],axis=0),window=params[3]))/(sliding_mean(np.mean(dirs_2_spike_rates[unit_2_start:unit_2_start+cond2_units-1],axis=0),window=params[3])+sliding_mean(np.mean(dirs_1_spike_rates[unit_1_start:unit_1_start+cond1_units-1],axis=0),window=params[3])),label=animal)
...
plt.plot(time,(all_anmimals_2-all_anmimals_1)/(all_anmimals_2+all_anmimals_1), linestyle=':', color='black', linewidth=4)
...
fig.savefig(save_name+'/All_animals_%i_Smoothed_%ims_window_change.png' % (np.size(dirs_animals_1[0]),params[1]))
plt.close('all')
...
if held_check == "Yes":
	...
	change_scored = (all_anmimals_2-all_anmimals_1)/(all_anmimals_2+all_anmimals_1)
	sem_change_scored = (sem(held_FRs_cond2_2,axis=0)-sem(held_FRs_cond1_2,axis=0))/(sem(held_FRs_cond2_2,axis=0)+sem(held_FRs_cond1_2,axis=0))
	...
	#Save figure
	fig.savefig(save_name+'/Held_all_animals_%i_Smoothed_%ims_window_change.png' % (np.size(dirs_animals_1[0]),params[1]))
	...
	#Save figure
	fig.savefig(save_name+'/Held_Stats_Smoothed_%ims_window_change.png' % (params[1]))
plt.close('all')
