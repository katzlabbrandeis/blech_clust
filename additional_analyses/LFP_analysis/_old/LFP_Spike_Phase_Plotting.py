	...
	for band_num in range(len(frequency_list)):
		for taste_num in	range(len(data_frame.taste.unique())):
			...
			for time_num in range(len(time_vector)-1):
				...
				#if len(queryidx) != 0 or len(queryidx) ==1:
				if len(queryidx) >= 2:
					#print( len(queryidx))
...
#Creates Heatmaps for density of spikes within phase over time
...
for unit in dframe_stat.unit.unique():
	...
	fig.suptitle('Unit %i' %(dframe_stat.unit.unique()[unit]),size=16)
