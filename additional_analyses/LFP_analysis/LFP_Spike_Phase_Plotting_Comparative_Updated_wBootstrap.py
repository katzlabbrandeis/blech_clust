...
if comp_check == "Yes":
	...
	#Ask about group comparisons
	msg   = "Did you already run this for the first group?"
	group_check = easygui.buttonbox(msg,choices = ["Yes","No"])

	if group_check == "No":
		#make list for storing change_score outputs
		change_outputs_1 = [[[ [] for col in range(len(grouped_df.band.unique()))] for taste in range(len(grouped_df.taste.unique()))]]
		chi_1 = np.zeros((len(grouped_df.band.unique()),len(grouped_df.taste.unique()),len(sorted(grouped_df.time_bin.unique())),4))

		#Grab details regarding cohort conditions from user
		group_conditions = easygui.multenterbox(msg = 'Input condition names:', fields = ['Cohort 1:','Cohort 2:'], values = ['Experimental','Control'])

	if group_check == "Yes":
		change_outputs_2 = [[[ [] for col in range(len(grouped_df.band.unique()))] for taste in range(len(grouped_df.taste.unique()))]]
		chi_2 = np.zeros((len(grouped_df.band.unique()),len(grouped_df.taste.unique()),len(sorted(grouped_df.time_bin.unique())),4))
...
# =============================================================================
	...
	#bootstrap subset of cells
	bootstap_iterations = 100;
	sample_subset = 0.8;

	#create output arrays for storage
	boot_cond1=np.zeros((4,4,bootstap_iterations,28))
	boot_cond2=np.zeros((4,4,bootstap_iterations,28))
	boot_diff=np.zeros((4,4,bootstap_iterations,28))

	#create column for uniuque
	grouped_df['unique_nrn'] = grouped_df['Animal'].astype(str)+'-'+grouped_df['unit'].astype(str)

	for iteration in trange(bootstap_iterations, desc = 'Bootstrap iteration'): #Iterates 100 times using 80% of the data
		iter_nrns = np.sort(np.random.choice(grouped_df.unique_nrn.unique(),
					size = int(len(grouped_df.unique_nrn.unique())*sample_subset),
					replace = False))
		boot_df = grouped_df.loc[grouped_df.unique_nrn.isin(iter_nrns)]

		for band in sorted(boot_df.band.unique()):
			for taste in sorted(boot_df.taste.unique()):
				query_1 = boot_df.query('taste == @taste and band ==@band and pval_cat == 0 and Condition == @conditions[0]')
				query_2 = boot_df.query('taste == @taste and band ==@band and pval_cat == 0 and Condition == @conditions[1]')

				#store outputs
				boot_cond1[band,taste,iteration] = np.array([query_1['time_bin'].value_counts()[x] \
	                    if x in query_1['time_bin'].unique() else 0 for x in \
	                    sorted(boot_df.time_bin.unique())])/\
	                    len(query_1.groupby(['Animal', 'unit']))*100

				boot_cond2[band,taste,iteration] = np.array([query_2['time_bin'].value_counts()[x] \
	                    if x in query_2['time_bin'].unique() else 0 for x in \
	                    sorted(boot_df.time_bin.unique())])/\
	                    len(query_1.groupby(['Animal', 'unit']))*100

				boot_diff[band,taste,iteration] = boot_cond2[band,taste,iteration] - boot_cond1[band,taste,iteration]
	...
	for band in sorted(grouped_df.band.unique()):
		...
		for taste in sorted(grouped_df.taste.unique()):
			...
			for tbin in range(len(sorted(grouped_df.time_bin.unique()))):
				 ...
				 if comp_check == "Yes":
					 if group_check == "No":
						 chi_1[band,taste,tbin] = chi_table.flatten()
					 if group_check == "Yes":
						 chi_2[band,taste,tbin] = chi_table.flatten()
			...
			if comp_check == "Yes":
				if group_check == "No":
					#store change_score outputs by band (Day 1 - Day 2)
					change_outputs_1[0][band][taste] = list((np.array([query_2['time_bin'].value_counts()[x] \
	                    if x in query_2['time_bin'].unique() else 0 for x in \
	                    sorted(grouped_df.time_bin.unique())])/\
	                    len(query_2.groupby(['Animal', 'unit']))*100) - (np.array([query_1['time_bin'].value_counts()[x] \
	                    if x in query_1['time_bin'].unique() else 0 for x in \
	                    sorted(grouped_df.time_bin.unique())])/\
	                    len(query_1.groupby(['Animal', 'unit']))*100))
				...
				if group_check == "Yes":
					#store change_score outputs by band (Day 1 - Day 2)
					change_outputs_2[0][band][taste] = list((np.array([query_2['time_bin'].value_counts()[x] \
	                    if x in query_2['time_bin'].unique() else 0 for x in \
	                    sorted(grouped_df.time_bin.unique())])/\
	                    len(query_2.groupby(['Animal', 'unit']))*100) - (np.array([query_1['time_bin'].value_counts()[x] \
	                    if x in query_1['time_bin'].unique() else 0 for x in \
...
if comp_check == "Yes":
	#Get name of directory where you want to save output files to
	comp_save_name = easygui.diropenbox(msg = 'Choose directory you want output files' +\
								   'sent to (and/or where ".dir" files are)')
	...
	try:
	    os.system('rm -r '+'./Comp_Phase_lock_analyses')
	except:
	    pass
	os.mkdir('./Comp_Phase_lock_analyses')


	for band in range(np.array(change_outputs_1).shape[1]):
		colors_new = plt.get_cmap('RdBu')(np.linspace(0, 1, 4))
		fig,axes = plt.subplots(nrows=2, ncols=2,sharex=True, sharey=True,
		        figsize=(15, 10), squeeze=False)
		fig.text(0.075, 0.5,'Change (% of Units from Day 1)', va='center',
		        rotation='vertical',fontsize=15)
		fig.text(0.5, 0.05, 'Time', ha='center',fontsize=15)
		axes_list = [item for sublist in axes for item in sublist]

		#make list for chisquare outputs
		comp_chi_stats = len(grouped_df.band.unique())*[[]]

		for taste in range(np.array(change_outputs_1).shape[2]):
			ax = axes_list.pop(0)

# =============================================================================
# 			#Separate conditional data
# 			cond_1 = change_outputs_1[0][band][taste][:]
# 			cond_2 = change_outputs_2[0][band][taste][:]
# =============================================================================
...
# 			for tbin in range(len(sorted(grouped_df.time_bin.unique()))):
#
# 				chi_table_comp = np.array([[((chi_1[band,taste,tbin,:][0]*chi_1[band,taste,tbin,:][3])\
# 					   -(chi_1[band,taste,tbin,:][2]*chi_1[band,taste,tbin,:][1])),\
# 						(chi_1[band,taste,tbin,:][1]*chi_1[band,taste,tbin,:][3])\
# 						-((chi_1[band,taste,tbin,:][0]*chi_1[band,taste,tbin,:][3])\
# 					   -(chi_1[band,taste,tbin,:][2]*chi_1[band,taste,tbin,:][1]))],\
# 					   [((chi_2[band,taste,tbin,:][0]*chi_2[band,taste,tbin,:][3])\
# 					   -(chi_2[band,taste,tbin,:][2]*chi_2[band,taste,tbin,:][1])),\
# 						(chi_2[band,taste,tbin,:][1]*chi_2[band,taste,tbin,:][3])\
# 						-((chi_2[band,taste,tbin,:][0]*chi_2[band,taste,tbin,:][3])\
# 					   -(chi_2[band,taste,tbin,:][2]*chi_2[band,taste,tbin,:][1]))]])
#
#
...
# # =============================================================================
#
# 				#comp_chi_stats[taste]=chi_stats[taste]+[tuple(chi2_contingency(chi_table_comp)[0:2])]
#
# 			#Create vector for sigvalues
# 			sig_vector =np.array([x[1] for x in chi_stats[taste]]) #find p values
# 			sig_vector= np.array([x*(x<=0.05) for x in sig_vector]) #convert all non-sig to 0
...
# =============================================================================
# 				plt.plot(sorted(grouped_df.time_bin.unique()),
#                 gaussian_filter1d(np.mean(np.array(boot_diff[0,0,:,:]),axis=0),sigma=1),
#                 color = 'black',linewidth = 2)
# =============================================================================
			#Computes the Kolmogorov-Smirnov statistic on 2 samples: two-sided
			#test for the null hypothesis that 2 independent samples are drawn
			#from the same continuous distribution
			sig_vector = [stats.ks_2samp(np.array(boot_diff[band,taste,:,x]), np.array(control_diff[band,taste,:,x]))[1] for x in range(0,len(sorted(grouped_df.time_bin.unique())))]
			sig_vector= np.array([x*(x<=0.05) for x in sig_vector]) #convert all non-sig to 0
			sig_vector[sig_vector == 0] = 'nan' #convert to nans for plotting

			#Applied a first order gaussian filter to smooth lines
			p1 = ax.plot(sorted(grouped_df.time_bin.unique()),
	                gaussian_filter1d(np.mean(np.array(boot_diff[band,taste,:,:]),axis=0),sigma=1.5),
	                color = 'salmon',linewidth = 3)
			p1_pos = ax.plot(sorted(grouped_df.time_bin.unique()),
                gaussian_filter1d(np.mean(np.array(boot_diff[band,taste,:,:]),axis=0)+np.std(np.array(boot_diff[band,taste,:,:]),axis=0),sigma=1.5),
                color = 'salmon',linewidth = 1,linestyle='dashed',alpha=0.8)
			p1_neg = ax.plot(sorted(grouped_df.time_bin.unique()),
                gaussian_filter1d(np.mean(np.array(boot_diff[band,taste,:,:]),axis=0)-np.std(np.array(boot_diff[band,taste,:,:]),axis=0),sigma=1.5),
                color = 'salmon',linewidth = 1,linestyle='dashed',alpha=0.8)

			p2 = ax.plot(sorted(grouped_df.time_bin.unique()),
	                gaussian_filter1d(np.mean(np.array(control_diff[band,taste,:,:]),axis=0),sigma=1.5),
	                color = 'midnightblue',linewidth = 3)
			p2_pos = ax.plot(sorted(grouped_df.time_bin.unique()),
	                gaussian_filter1d(np.mean(np.array(control_diff[band,taste,:,:]),axis=0)+np.std(np.array(control_diff[band,taste,:,:]),axis=0),sigma=1.5),
	                color = 'midnightblue',linewidth = 1,linestyle='dashed',alpha=0.8)
			p2_neg = ax.plot(sorted(grouped_df.time_bin.unique()),
	                gaussian_filter1d(np.mean(np.array(control_diff[band,taste,:,:]),axis=0)-np.std(np.array(control_diff[band,taste,:,:]),axis=0),sigma=1.5),
	                color = 'midnightblue',linewidth = 1,linestyle='dashed',alpha=0.8)

			p3 = ax.plot(sorted(grouped_df.time_bin.unique()),
	                [0 for x in range(len(sorted(grouped_df.time_bin.unique())))] ,color = 'black',linewidth = 2,linestyle='dashed',alpha=0.7)

			#Find max y value for sig plotting and update sigvector vals
			max_y = np.max([np.array(boot_diff[band,taste,:,:]),np.array(control_diff[band,taste,:,:])])

			sig_vector[sig_vector >= 0] = np.ceil(max_y+1) #convert to nans for plotting

			#annotate plot
# =============================================================================
# 			p4 = ax.scatter(sorted(grouped_df.time_bin.unique()),sig_vector,c="g", s=80, alpha=0.5, marker=r'$\clubsuit$',
# 	            label="Luck")
# =============================================================================
			...
			ax.set_ylim(-19,19)
		fig.suptitle([x[0] for x in list(freq_vals.values())][band],size=15,y=1)
		fig.legend(group_conditions,loc = (0.42, 0), ncol=4)
		fig.subplots_adjust(hspace=0.25,wspace = 0.05)
		plt.suptitle('%s Sig. Spike-Phase Locking\n %s: %i cells,  %s: %i cells' %([x[0] for x in list(freq_vals.values())][band],conditions[0],len(query_1.groupby(['Animal', 'unit'])),conditions[1],len(query_2.groupby(['Animal', 'unit']))),size=18,fontweight="bold")
		fig.savefig('./Comp_Phase_lock_analyses/'+'%s_SPL_N_%i_animals.png' %([x[0] for x in list(freq_vals.values())][band], len(grouped_df.Animal.unique())))
		plt.close(fig)
...
# =============================================================================
			...
			#from the same continuous distribution
# =============================================================================
# 		sig_vector = [stats.ks_2samp(np.array(boot_diff[band,:,:,x]), np.array(control_diff[band,:,:,x]))[1] for x in range(0,len(sorted(grouped_df.time_bin.unique())))]
# 		sig_vector= np.array([x*(x<=0.05) for x in sig_vector]) #convert all non-sig to 0
# 		sig_vector[sig_vector == 0] = 'nan' #convert to nans for plotting
#
# =============================================================================
		#Applied a first order gaussian filter to smooth lines
		p1 = ax.plot(sorted(grouped_df.time_bin.unique()),
                gaussian_filter1d(np.mean(np.mean(np.array(boot_diff[band,:,:,:]),axis=0),axis=0),sigma=1.5),
                color = 'salmon',linewidth = 3)
		p1_pos = ax.plot(sorted(grouped_df.time_bin.unique()),
                gaussian_filter1d(np.mean(np.mean(np.array(boot_diff[band,:,:,:]),axis=0),axis=0)+np.std(np.std(np.array(boot_diff[band,:,:,:]),axis=0),axis=0),sigma=1.5),
                color = 'salmon',linewidth = 1,linestyle='dashed',alpha=0.8)
		p1_neg = ax.plot(sorted(grouped_df.time_bin.unique()),
                gaussian_filter1d(np.mean(np.mean(np.array(boot_diff[band,:,:,:]),axis=0),axis=0)-np.std(np.std(np.array(boot_diff[band,:,:,:]),axis=0),axis=0),sigma=1.5),
                color = 'salmon',linewidth = 1,linestyle='dashed',alpha=0.8)
		...
		p2 = ax.plot(sorted(grouped_df.time_bin.unique()),
                gaussian_filter1d(np.mean(np.mean(np.array(control_diff[band,:,:,:]),axis=0),axis=0),sigma=1.5),
                color = 'midnightblue',linewidth = 3)
		p2_pos = ax.plot(sorted(grouped_df.time_bin.unique()),
                gaussian_filter1d(np.mean(np.mean(np.array(control_diff[band,:,:,:]),axis=0),axis=0)+np.std(np.std(np.array(control_diff[band,:,:,:]),axis=0),axis=0),sigma=1.5),
                color = 'midnightblue',linewidth = 1,linestyle='dashed',alpha=0.8)
		p2_neg = ax.plot(sorted(grouped_df.time_bin.unique()),
                gaussian_filter1d(np.mean(np.mean(np.array(control_diff[band,:,:,:]),axis=0),axis=0)-np.std(np.std(np.array(control_diff[band,:,:,:]),axis=0),axis=0),sigma=1.5),
                color = 'midnightblue',linewidth = 1,linestyle='dashed',alpha=0.8)
