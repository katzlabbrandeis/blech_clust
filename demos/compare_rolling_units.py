...
def compare_datasets(rolling_dir, non_rolling_dir, output_dir=None):
    ...
    print(
        f"  Difference:         {rolling_data['n_units'] - non_rolling_data['n_units']}")
    ...
    ax2 = axes[0, 1]
    ax2.hist(rolling_wf_counts, bins=30, alpha=0.7,
             label='Rolling', color='skyblue')
    ax2.hist(non_rolling_wf_counts, bins=30, alpha=0.7,
             label='Non-Rolling', color='lightcoral')
    ...
    box_plot = ax3.boxplot(data_to_plot, labels=[
                           'Rolling', 'Non-Rolling'], patch_artist=True)
    ...
    ax4 = axes[1, 1]
    if rolling_data['classifier_probs'] and non_rolling_data['classifier_probs']:
        ax4.hist(rolling_probs, bins=20, alpha=0.7,
                 label='Rolling', color='skyblue')
        ax4.hist(non_rolling_probs, bins=20, alpha=0.7,
                 label='Non-Rolling', color='lightcoral')
    ...
    if output_dir:
        plt.savefig(os.path.join(output_dir, 'rolling_comparison.png'),
...
def main():
    ...
    if not os.path.exists(args.non_rolling_dir):
        print(
            f"Error: Non-rolling directory does not exist: {args.non_rolling_dir}")
        sys.exit(1)
