        ...
        for i in range(n_channels):
            ...
            for spike_time in spike_times:
                ...
                if i == 0:  # Negative spikes
                    spike_waveform = -500 * \
                        np.exp(-t_spike**2 / 0.2) * np.sin(np.pi * t_spike)
...
def test_new_layout():
    ...
    try:
        ...
        snippet_window_duration = main_window_duration * \
            plotter.snippet_window_ratio  # 0.4 seconds

        print(f"\nWindow Calculations:")
        print(f"  - Main window duration: {main_window_duration} seconds")
        print(
            f"  - Snippet window duration: {snippet_window_duration} seconds")
        ...
        if len(snippets) > 0:
            ...
            print(
                f"  - Snippet length: {actual_samples} samples (expected: {expected_samples})")
            print(
                f"  - Snippet duration: {actual_samples/sampling_rate:.3f} seconds")
