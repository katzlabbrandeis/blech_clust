...
def create_test_data_with_spikes():
    ...
    with h5py.File(h5_path, 'w') as f:
        ...
        for i in range(n_channels):
            ...
            signal = np.random.normal(0, 20, n_samples)  # Lower noise for clearer spikes
            ...
            spike_times = spike_times[(spike_times > 100) & (spike_times < n_samples - 100)]  # Avoid edges
            ...
            for spike_time in spike_times:
                ...
                if i % 2 == 0:  # Negative spikes
                    spike_waveform = -300 * np.exp(-t_spike**2 / 0.1) * np.sin(np.pi * t_spike)
            ...
            lfp_component = 10 * np.sin(2 * np.pi * 8 * time_array)  # 8 Hz oscillation
...
def test_snippet_extraction():
    ...
    try:
        ...
        print(f"  - Negative snippets: {sum(1 for p in snippet_polarities if p == -1)}")
        print(f"  - Positive snippets: {sum(1 for p in snippet_polarities if p == 1)}")
        
        if len(snippets) > 0:
            print(f"  - Snippet length: {len(snippets[0])} samples")
            print(f"  - Expected length: {int((plotter.snippet_before_ms + plotter.snippet_after_ms) / 1000.0 * data_loader.sampling_rate)} samples")
