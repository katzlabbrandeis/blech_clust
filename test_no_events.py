...
def create_test_data():
    ...
    data = 20 * np.sin(2 * np.pi * 10 * time) + np.random.normal(0, 5, n_samples)
...
def test_viewer_without_events():
    ...
    try:
        ...
        print(f"✅ Snippet extraction works (found {len(viewer.current_snippets)} snippets)")
        ...
        assert not hasattr(viewer, '_on_scroll') or not callable(getattr(viewer, '_on_scroll', None)), "Custom scroll handler should be removed"
        assert not hasattr(viewer, '_on_key_press') or not callable(getattr(viewer, '_on_key_press', None)), "Custom key handler should be removed"
        print("✅ Custom event handlers properly removed")
        
        # Test that double-click still works (this should remain)
        assert hasattr(viewer, '_on_click') and callable(viewer._on_click), "Double-click handler should be preserved"
