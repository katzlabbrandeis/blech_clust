...
class UnitExplorer:
    def __init__(self, h5_path):
        ...
        self.umap_embedding = self.umap_reducer.fit_transform(
            self.unit_means_scaled)
    ...
    def plot_unit_waveforms(self, unit_idx):
        ...
        # Plot mean waveform
        self.ax_waveform.plot(time_points, unit_mean,
        ...
        plot_indices = np.random.choice(
            unit_data.shape[0], n_plot_waveforms, replace=False)
        for i in plot_indices:
            self.ax_waveform.plot(
                time_points, unit_data[i], 'gray', alpha=0.1, linewidth=0.5)
