...
class TestMain:
    ...
    def test_main_function_flow(self, mock_generate_scripts, mock_plot_digins,
        ...
        mock_get_file_lists.return_value = {
            'one file per channel': {'electrodes': ['amp-001.dat']}}

        dig_handler = MagicMock()
        mock_dig_handler.return_value = dig_handler

        mock_process_file_info.return_value = (
            ['amp-001.dat'], 30000, 1000, None)
        ...
        pipeline_check.write_to_log.assert_called_with(
            mock_pipeline_check.return_value.check_previous.return_value, 'completed')
