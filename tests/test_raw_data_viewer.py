...
class TestRawDataViewerIntegration(unittest.TestCase):
    ...
    def _create_test_hdf5(self):
        ...
        with tables.open_file(self.hdf5_path, 'w') as hf5:
            ...
            for i in range(2):
                ...
                emg_signal += 100 * np.sin(2 * np.pi * 20 * t)  # EMG-like frequency
    ...
    def test_main_with_config(self, mock_app_class):
        ...
        call_args = mock_app_class.call_args[0][0]  # First positional argument (config)
        self.assertEqual(call_args['hdf5_path'], self.hdf5_path)
        self.assertEqual(call_args['window_duration'], 3.0)  # Overridden by command line
        self.assertEqual(call_args['threshold'], 200)  # Overridden by command line
    ...
    def test_save_config(self):
        ...
        test_config_save_path = os.path.join(self.temp_dir, 'saved_config.json')
